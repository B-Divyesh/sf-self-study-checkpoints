...
Open <https://self-study-checkpoints.sociobot.in/?demo=1> or use `?demo=1` locally. `/demo` opens the same isolated sample.
...
When a review link is copied from the sample, it uses `?demo=1&review=…` so both sides stay inside the isolated demo namespace.
